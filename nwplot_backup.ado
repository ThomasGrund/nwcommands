...
program nwplot
	...
	local aspectratio = `aspectratio'*  0.8
	...
	local symbolorder ""
	local symbollabels ""
	if ("`symbol'" != "" & substr(trim("`symbol'"),1,1) != ","){
	...
	if ("`size'" != ""){
		...
		local sizekeys "`r(min)' `r(max)'"
		local sizekeys_size "`r(min)' `r(max)'"
	...
	if "`edgesize'" != "" {
		...
		local edgesize_legendoff "`legendoff'"
		if "`forcekeys'" == "" {
	...
	if "`edgecolor'" != "" {
		local 0 = "`edgecolor'"
		syntax anything, [forcekeys(string)]
	...
	if ("`layout'"=="mds"  ){
		...
		forvalues i = 1/`components' {
			...
			if `i' == 1  & `components' == 1 {
				mata: Coord_comp[.,1] = Coord_comp[.,1] :+ 0.175
				// HERE 0.125
				// CENTRALIZE
			}
			
			// Deal with second to fifth largest component
			if `i' >= 2 & `i' <= 5 {
				mata: Coord_comp[.,1] = (Coord_comp[.,1] :*0.125):+ 1.075
				mata: Coord_comp[.,2] = (Coord_comp[.,2] :*0.25):+ ( 1 - (`i'-1)*0.25)
			}
	...
	local columns = max(`symbolkeys_num', `colorkeys_num', `sizekeys_num', `edgesizekeys_num')
	...
	if "`symbolkeys_legendoff'" == "" {
		if `symbolkeys_num' > 1 {	
			...
			foreach j in `symbolkeys' {
				...
				local ghostcmd `"`ghostcmd' || (scatter `ghost1' `ghost2' if `ghost1' !=.,  msymbol("`tempsymbol'")  mcolor("scheme p0")  msize(2) `scatteropt') "'            		
			}
		}
	}
	else {
	...
	if "`size'" != "" & "`sizekeys_legendoff'" == ""{
		...
		foreach szkey in `sizekeys' {
			...
			local ghostcmd `"`ghostcmd' || (scatter `ghost1' `ghost2' if `ghost1' !=.,  msymbol("scheme p0")  mlcolor("scheme p0") mfcolor("scheme background")  msize(`szkey_size') `scatteropt') "'            		
		}
		local sizekeys_num : word count `sizekeys'
	}
	else {
	...
	if "`edgesizekeys'" != "" & "`edgesizekeys_legendoff'" == ""{
		foreach eszkey in `edgesizekeys' {
			...
			local ghostcmd `"`ghostcmd' || (`pc' `ghost1' `ghost2' `ghost2' `ghost1' if `ghost1' !=., lwidth(`tempval_line') lcolor("scheme p0") mfcolor("scheme p0") mlcolor("scheme p0") msize(`tempval_arrow') barbsize(`tempval_barb') `lineopt') ||"'
		}
		local edgesizekeys_num : word count `edgesizekeys'
	}
	else {
	...
	/*
	// Ghost plots for edge color
	local edgecolorkeys_num : word count `edgecolorkeys'
	if `edgecolorkeys_num' > 1 {	
		local eckey = 0
		foreach i in `edgecolorkeys' {
			_getcolorstyle, i(`i') edgecolorpalette(`edgecolorpalette') edgepatternpalette(`edgepatternpalette') scheme(`scheme')
			local temppattern = r(edgepattern)
			local tempcolstyle = r(edgecol)
			local ghostcmd `"`ghostcmd' || (`pc' `ghost1' `ghost2' `ghost2' `ghost1' if `ghost1' !=., mcolor("`tempcolstyle'")  lcolor("`tempcolstyle'") lwidth(2) msize(2) ) "'            		
		}
	}
	else {
		local edgecolorkeys_num = 0
	}*/
	...
	if "`legendopt'" == "" | `keysused' == 0 {
		...
		local legendcmd `"legend(order(`colororder' `colorident' `symbolorder' `symbolident' `sizeorder' `sizeident' `edgesizeorder' `edgesizeident') `colorlabels' `symbollabels' `sizelabels' `edgesizelabels' `legendopt')"'
	}
	...
	forvalues tempsiz_mat = 1/`tempsize_rows'{
		...
		forvalues i = 1/`ncols' {
			...
			forvalues j = 1/`symbs'{
				...
				local scattercmd `"`scattercmd' (scatter ny nx if ncolor == `tempcol' & nsymbol == `tempsymb' & nsize == `tempsiz',  msymbol("`tempsymbol'") mlwidth(vthin) mlcolor(`tempcolstyle_line') mfcolor("`tempcolstyle_fill'") msize(`tempsiz_node') `scatterlabel' `scatteropt') ||"' 
			}
		}
	}
	...
	local graphcmd `"twoway `ghostcmd' `pccmd' `scattercmd' `pccmdforeground', ylabel(, nogrid) yscale(off range(0 100)) xscale(off range(0 125)) graphregion(color("scheme plotregion")) plotregion(color("scheme plotregion") margin(`pmargin' `pmargin' `pmargin' `pmargin')) aspectratio(`aspectratio') `legendcmd' `schemetwoway' `twowayopt'"' 
