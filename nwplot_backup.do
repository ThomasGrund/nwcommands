...
program nwplot
	...
	if ("`color'" != ""){
		local 0 = "`color'"
		syntax varlist(min=1 max=1) [, norescale forcekeys(string)]
		...
		if "`rescale'" == "" {	
			...
			if "`forcekeys'" == "" {
				qui tab `varlist', matrow(colorkeysmap)
				forvalues i = 1/`r(r)' {
	...
	if ("`symbol'" != ""){
		...
		if "`rescale'" == "" {	
			...
			if "`forcekeys'" == "" {
				qui tab `varlist', matrow(symbolkeysmap)
				forvalues i = 1/`r(r)' {
					local j = `i' + `keysused'
	...
	local keysused_2 : word count `symbolkeys'
	local keysused = `keysused' + `keysused_2'
	...
	if "`edgesize'" != "" {
		...
		local edgesize_minval = `r(minval)'
		local edgesize_maxval = `r(maxval)'
	}
	else {
	...
	local edgecolorkeys "1 2 3"
	local edgecolororder "4 5 6"
	local edgecolorlabels `"label(4 "e_a") label(5 "e_b") label(6 "e_c")"' 
	...
	if `cols_found' == 0 {
		local cols = max(`symbolkeys_num', `colorkeys_num') + 1
		local legendopt "`legendopt' cols(`cols')"
	}
	...
	if "`legendopt'" == "" | `keysused' == 0 {
		...
		local legendcmd `"legend(order(`colororder' - `symbolorder' - `edgecolororder') `colorlabels' `symbollabels' `edgecolorlabels' `legendopt')"'
	}
	...
	tab edgecolor, matrow(edgecolorrow)
	...
	local graphcmd `"twoway `ghostcmd' `pccmd' `scattercmd' `pccmdforeground', ylabel(, nogrid) yscale(off range(0 100)) xscale(off range(0 125)) graphregion(color("scheme plotregion")) plotregion(color("scheme plotregion") margin(`pmargin' `pmargin' `pmargin' `pmargin')) aspectratio(`aspectratio') `legendcmd' `options' `schemetwoway'"' 
